...
//   3. Seed global reference data (categories, suppliers, products)
//   4. Wipe blob-owned per-event children only (topups, wastage)
//   5. Seed missing v2-owned rows from blob (never delete or overwrite
//      existing relational data — see CUTOVER 2026-06-08)
//   6. Stamp events.synced_at / source_updated_at
//
// CUTOVER (2026-06-08): the merged app (v2.html) owns event operations,
// recipes, suppliers, categories, warehouses, and the product library.
// Blob sync is additive only for those tables.
//
...
async function ensureCategories(names) {
  ...
  // CUTOVER: only insert categories that don't exist — v2 Settings owns colours.
  const all = await sb.get('categories', '?select=id,name');
  const have = new Set(all.map((c) => c.name));
  const toInsert = rows.filter((r) => !have.has(r.name));
  if (toInsert.length) await sb.insert('categories', toInsert);
  const refreshed = await sb.get('categories', '?select=id,name');
  const map = {};
  for (const c of refreshed) map[c.name] = c.id;
  return map;
}
...
async function ensureSuppliers(suppliersBlob, productsBlob) {
  ...
  // CUTOVER: only insert suppliers that don't exist — v2 Suppliers panel owns SOR etc.
  const all = await sb.get('suppliers', '?select=id,name');
  const have = new Set(all.map((s) => s.name));
  const toInsert = rows.filter((r) => !have.has(r.name));
  if (toInsert.length) await sb.insert('suppliers', toInsert);
  const refreshed = await sb.get('suppliers', '?select=id,name');
  const map = {};
  for (const s of refreshed) map[s.name] = s.id;
  return map;
}
...
async function ensureProducts(productsBlob, catMap, supMap) {
  ...
  for (const p of productsBlob) {
    ...
    if (knownMap[legPid]) {
      ...
      // CUTOVER (2026-06-07): the products library is now v2-owned. Once a
      // product exists in v2 (mapped here), the sync engine NEVER overwrites
      // its mutable fields (name, case_price, units_per_case, …) from the
      // blob — those are edited directly in the v2 app (v2.html → DB.products).
      // We still INSERT brand-new products below so event_products FKs resolve,
      // but established rows are left untouched. Do NOT add an update() here.
      continue;
    }
...
async function ensureEventRow(legacy, blob) {
  ...
  if (eventUuid) {
    // CUTOVER (2026-06-07): event details (name, venue, venue_postcode,
    // start_date, end_date, status) are now v2-owned and edited directly in
    // the merged app (v2.html → Event Setup). Once an event exists in v2 the
    // blob sync NEVER overwrites those fields — only the seed INSERT below
    // populates them. We still resolve linked_event_id (sync bookkeeping)
    // further down. Do NOT re-introduce an update() of the mutable fields.
...
async function wipeEventChildren(eventUuid) {
  // CUTOVER (2026-06-08): v2-owned tables are NEVER wiped by blob sync.
  // Only topups + wastage remain blob-owned until their panels are ported.
  await sb.delete('topup_sessions', `event_id=eq.${eventUuid}`);
  await sb.delete('wastage_batches', `event_id=eq.${eventUuid}`);
}

async function eventHasAny(table, eventUuid, filterCol = 'event_id') {
  const rows = await sb.get(
    table,
    `?${filterCol}=eq.${eventUuid}&select=id&limit=1`
  );
...
async function insertBars(eventUuid, legacyEventId, blobBars) {
  const existing = await sb.get('bars', `?event_id=eq.${eventUuid}&select=id,name`);
  const map = {};
  for (const b of existing) map[b.name] = b.id;

  if (!blobBars || !blobBars.length) return map;

  // CUTOVER: seed missing bar names only — never delete or rename v2 bars.
  const seen = new Map();
  const rows = [];
  for (const raw of blobBars) {
    const name = String(raw || '').trim();
    if (!name || seen.has(name) || map[name]) continue;
    seen.set(name, true);
    rows.push({ event_id: eventUuid, name });
  }
...
async function insertRecipients(eventUuid, legacyEventId, blobRecipients) {
  const existing = await sb.get('recipients', `?event_id=eq.${eventUuid}&select=id,name`);
  const map = {};
  for (const r of existing) map[r.name] = r.id;

  if (!blobRecipients || !blobRecipients.length) return map;
...
async function insertEventProducts(eventUuid, blob, prodMap) {
  ...
  const existing = await sb.get(
    'event_products',
    `?event_id=eq.${eventUuid}&select=product_id`
  );
  ...
  for (const p of products) {
    ...
    rows.push({
      ...
      order_price_override:      null,
      order_unit_price_override: null,
...
async function insertDistribution(eventUuid, blob, prodMap, barMap) {
  ...
  const existingMenu = await sb.get(
    'bar_products',
    `?event_id=eq.${eventUuid}&select=bar_id,product_id`
  );
  const existingDist = await sb.get(
    'distribution',
    `?event_id=eq.${eventUuid}&select=bar_id,product_id`
  );
  const menuHave = new Set(existingMenu.map((r) => `${r.bar_id}|${r.product_id}`));
  const distHave = new Set(existingDist.map((r) => `${r.bar_id}|${r.product_id}`));
  ...
  for (const [legPid, perBar] of Object.entries(dist)) {
    ...
    for (const [barName, qty] of Object.entries(perBar || {})) {
      ...
      const q = Number(qty);
      if (Number.isFinite(q) && q > 0 && !distHave.has(k)) {
...
async function insertCounts(eventUuid, blob, prodMap, barMap) {
  // CUTOVER: once v2 has any count sessions, blob counts are ignored.
  if (await eventHasAny('stock_counts', eventUuid)) return;
...
async function insertTransfers(eventUuid, blob, prodMap, recMap) {
  ...
  // CUTOVER: v2 transfer log owns all transfer types (warehouse, bar, etc.).
  const existing = await sb.get(
    'transfers',
    `?or=(from_event_id.eq.${eventUuid},to_event_id.eq.${eventUuid})&select=id&limit=1`
  );
...
async function insertClosing(eventUuid, blob, prodMap) {
  ...
  for (const [legPid, c] of Object.entries(closing)) {
    ...
    if (c.closingCases != null || c.closingSingles != null) {
      // close_count is expressed in CASES (matching the live app's
      // fullCount = cases + singles/units_per_case). Singles are a
      // fraction of a case, so divide — do NOT multiply cases up to units.
      const upc = upcByProduct[productId] || 1;
      closeCount = cases + (upc > 0 ? singles / upc : 0);
...
async function insertTillSales(eventUuid, blob) {
  ...
  if (await eventHasAny('till_imports', eventUuid)) return;
...
async function insertModifierSales(eventUuid, blob) {
  ...
  if (await eventHasAny('modifier_imports', eventUuid)) return;
...
function syncLockKey(legacyId) {
  return `event:${legacyId}`;
}

/** One full wipe-and-rebuild pass (caller must hold the per-event lock). */
async function syncEventOnce(legacyId) {
  // 1. Fetch blob
...
export async function syncEvent(legacyId) {
  ...
  const lockKey = syncLockKey(legacyId);
  if (!await sb.tryAcquireSyncLock(lockKey)) {
    return {
      ok: true,
      skipped: true,
  ...
  try {
    let result = await syncEventOnce(legacyId);

    // Blob may have advanced while we were rebuilding — one trailing pass.
    const fresh = await sb.get(
      'stock_events',
      `?id=eq.${enc(legacyId)}&select=updated_at`
    );
    const latest = fresh[0]?.updated_at;
    if (latest && result.blob_updated_at && latest !== result.blob_updated_at) {
      result = await syncEventOnce(legacyId);
    }
    return result;
  } finally {
    await sb.releaseSyncLock(lockKey).catch(() => {});
  }
}
...
export async function syncRecipes() {
  ...
  // CUTOVER (2026-06-08): recipes are v2-owned (Square panel). Seed missing
  // till items only — never delete or overwrite existing recipes.
  const existing = await sb.get('recipes', '?select=id,till_item,till_variation');
  const have = new Set(
    existing.map((r) =>
      `${String(r.till_item || '').trim().toLowerCase()}|${String(r.till_variation || '').trim().toLowerCase()}`
    )
  );
  ...
  for (const r of recipes) {
    ...
    const tillVariation = String(r.tillVariation || '').trim();
    const key = `${tillItem.toLowerCase()}|${tillVariation.toLowerCase()}`;
    if (have.has(key)) continue;

    const [rec] = await sb.insert('recipes', [{
      till_item:      tillItem,
      till_variation: tillVariation,
  ...
  return { ok: true, recipeCount: recipes.length, created };
}
