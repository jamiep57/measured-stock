...
--
-- For each unique-constrained table the update + delete run in a SINGLE
-- statement via a data-modifying CTE, so the `target` set (the row that
-- survives per group: the keeper's row if present, else the lowest-id
-- duplicate) is shared and the snapshot is consistent. The UPDATE moves
-- the survivor to the keeper and writes summed quantities; the DELETE
-- removes the remaining duplicate rows (never the keeper, never the
-- survivor).
--
...
-- =====================================================================
-- DONE. This mutates data; call it from the app's "Merge duplicates" UI,
-- or manually:
