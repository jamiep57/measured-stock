-- =====================================================================
-- 020 — Supplier purchase options (multiple prices per supplier)
-- =====================================================================
-- A product can now have several rows for the SAME supplier when the
-- pack size / SKU differs (e.g. Vinca PET £17.88 and Vinca Cans £18.12).
--
-- Changes:
--   • product_suppliers gains pack_size + units_per_case
--   • UNIQUE(product_id, supplier_id) → UNIQUE(product_id, supplier_id,
--     pack_size, sku)
--   • merge_products() accepts optional p_fields jsonb for per-field
--     keeper choices and folds every supplier offer onto the result
--
-- Apply: AFTER 019. Idempotent: yes.
-- =====================================================================

-- ---------- Schema: purchase-option columns ---------------------------
...
-- Backfill pack size from the parent product's case_size.
UPDATE public.product_suppliers ps
   SET pack_size = coalesce(nullif(trim(ps.pack_size), ''), nullif(trim(p.case_size), ''), ''),
       units_per_case = coalesce(ps.units_per_case, p.units_per_case)
  FROM public.products p
 WHERE ps.product_id = p.id
   AND (ps.pack_size = '' OR ps.units_per_case IS NULL);

UPDATE public.product_suppliers SET sku = coalesce(sku, '') WHERE sku IS NULL;
...
DROP INDEX IF EXISTS public.uq_product_suppliers_offer;

ALTER TABLE public.product_suppliers
  DROP CONSTRAINT IF EXISTS product_suppliers_offer_key;

ALTER TABLE public.product_suppliers
  ADD CONSTRAINT product_suppliers_offer_key
  UNIQUE (product_id, supplier_id, pack_size, sku);

-- ---------- merge_products (3-arg, field choices + all offers) --------

DROP FUNCTION IF EXISTS public.merge_products(uuid, uuid[]);
...
-- =====================================================================
-- DONE. Verify two offers for the same supplier on one product:
--   SELECT s.name, ps.pack_size, ps.case_price, ps.is_preferred
--     FROM product_suppliers ps JOIN suppliers s ON s.id = ps.supplier_id
--    WHERE ps.product_id = '<uuid>' ORDER BY s.name, ps.pack_size;
-- =====================================================================
