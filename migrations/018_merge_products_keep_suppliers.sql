-- =====================================================================
-- 018 — merge_products(): keep every supplier when folding duplicates
-- =====================================================================
-- Migration 015 wrote merge_products() BEFORE migration 016 added the
-- product_suppliers table. Because product_suppliers.product_id is
-- ON DELETE CASCADE, the old function silently destroyed the suppliers
-- attached to any duplicate when it deleted the duplicate product rows —
-- so a merge kept only the keeper's suppliers/prices.
--
-- This version re-points the duplicates' product_suppliers rows onto the
-- keeper BEFORE the duplicates are deleted, folding the two supplier lists
-- into one. Rules:
--   • A supplier present only on a duplicate is carried over to the keeper.
--   • A supplier present on both keeps a single row; the keeper's prices/SKU
--     win, and any gap (NULL) is filled from the duplicate.
--   • Duplicates that share a supplier are de-duped (earliest row wins).
--   • Exactly one preferred row survives: the keeper's preferred supplier if
--     it had one, otherwise a duplicate's — matching the single-preferred
--     partial unique index and the legacy-column sync from migration 016.
--
-- Everything else (event_products, closing_stock, distribution,
-- warehouse_stock, bar_products and the line tables) is unchanged from 015.
--
-- Apply: AFTER 016 (needs product_suppliers). Idempotent: yes (CREATE OR
-- REPLACE). Runs in an implicit transaction — all-or-nothing.
-- =====================================================================
...
-- =====================================================================
-- DONE. Verify a merge keeps both suppliers:
--   SELECT s.name, ps.case_price, ps.unit_price, ps.is_preferred
--     FROM product_suppliers ps JOIN suppliers s ON s.id = ps.supplier_id
--    WHERE ps.product_id = '<keep-uuid>' ORDER BY ps.is_preferred DESC, s.name;
-- =====================================================================
