...
function renderBarPills(bars, onOpen) {
  ...
  wrap.querySelectorAll('[data-bar-id]').forEach((btn) => {
    btn.onclick = async () => onOpen(btn.dataset.barId);
  });
  $('setupAddBar')?.addEventListener('click', async () => onOpen(null));
}
...
function renderRecipientPills(recipients, onOpen) {
  ...
  wrap.querySelectorAll('[data-recip-id]').forEach((btn) => {
    btn.onclick = async () => onOpen(btn.dataset.recipId);
  });
  $('setupAddRecip')?.addEventListener('click', async () => onOpen(null));
}
...
export async function mountSetupPanel(route, state = { events: [] }) {
  ...
  function wireEventImage() {
    const fileInput = $('setupImageFile');
    $('setupImagePick')?.addEventListener('click', async () => fileInput?.click());
    fileInput?.addEventListener('change', () => {
      const file = fileInput.files?.[0];
      if (file) saveEventImage(file);
    });
    $('setupImageClear')?.addEventListener('click', async () => clearEventImage());
  }
  ...
  async function openBarForm(barId) {
    const bar = barId ? (event.bars || []).find((b) => b.id === barId) : null;
    openSheet({
      title: bar ? 'Edit bar' : 'Add bar',
      variant: 'admin-full',
  ...
  async function openRecipientForm(recipId) {
    const r = recipId ? (event.recipients || []).find((x) => x.id === recipId) : null;
    openSheet({
      title: r ? 'Edit recipient' : 'Add recipient',
      variant: 'admin-full',
