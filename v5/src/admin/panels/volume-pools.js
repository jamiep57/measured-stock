...
export function mountVolumePoolsPanel() {
  const listEl = $('vpList');
  ...
  function wireDetail() {
    ...
    $('vpEditBtn')?.addEventListener('click', () => openPoolForm(currentPool()?.name));
    ...
    detailBody.querySelectorAll('[data-vp-remove]').forEach((btn) => {
      btn.addEventListener('click', () => removeMember(btn.dataset.vpRemove));
    });
    ...
    mountAddSearch();
    $('vpAddBtn')?.addEventListener('click', () => addMember());
  }
  ...
  async function removeMember(productId) {
    const product = products.find((p) => p.id === productId);
    if (!product) return;
    if (!confirm(`Remove “${product.name}” from this volume pool?`)) return;
    try {
  ...
  async function addMember() {
    ...
    if (product?.pool_name && normPoolName(product.pool_name) !== pool.key) {
      const ok = confirm(
        `“${product.name}” is already in pool “${product.pool_name}”. Move it to “${pool.name}”?`,
      );
      if (!ok) return;
    }
    const btn = $('vpAddBtn');
  ...
  async function saveNewPool(editName) {
    ...
    try {
      ...
      if (product?.pool_name && normPoolName(product.pool_name) !== key) {
        const ok = confirm(
          `“${product.name}” is already in pool “${product.pool_name}”. Move it here?`,
        );
        if (!ok) return;
      }
  ...
  async function deletePool(name) {
    if (!confirm(`Delete volume pool “${name}”? Products stay in the library but are unlinked.`)) return;
    try {
  ...
  function openPoolForm(editName) {
    ...
    $('vpSave').onclick = () => saveNewPool(editing ? editName : null);
