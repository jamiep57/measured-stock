...
export async function mountKitLibraryPanel() {
  ...
  function openKitForm(editId) {
    ...
    $('kitLibImagePick')?.addEventListener('click', async () => fileInput?.click());
    $('kitLibImageFind')?.addEventListener('click', async () => {
    ...
    $('kitLibImageSearchBtn')?.addEventListener('click', async () => runImageSearch());
    ...
    $('kitLibImageClear')?.addEventListener('click', async () => {
  ...
  async function startAutoPhotoRun(targets, { skipExisting }) {
    ...
    $('kitAutoPhotoCancel').onclick = async () => {
      cancelled = true;
      if (statusEl) statusEl.textContent = 'Cancelling after current item…';
    };
    $('kitAutoPhotoSkip').onclick = async () => {
      skipCurrent = true;
  ...
  function openCategoriesManager() {
    function paintCatList() {
      ...
      el.querySelectorAll('[data-edit-cat]').forEach((btn) => {
        btn.onclick = async () => openCategoryForm(btn.dataset.editCat);
      });
    }
    ...
    $('kitCatAdd').onclick = async () => openCategoryForm(null);
  }
  ...
  async function openLabelPrintQueue() {
    ...
    $('kitLabelQueueClose').onclick = async () => closeModal();
  ...
  async function openMobileCountPairingAsync() {
    ...
    $('kitMobileCountClose').onclick = async () => closeModal();
    $('kitMobileCountOpen').onclick = async () => {
  ...
  async function openKitForm(editId) {
    const p = editId ? products.find((x) => x.id === editId) : null;
    const stock = p ? (Number(stockMap.get(p.id)) || 0) : 0;
    const catOpts = [
      '<option value="">— none —</option>',
  ...
  async function openCategoryForm(editId) {
    const c = editId ? categories.find((x) => x.id === editId) : null;
    const colourOpts = CATEGORY_COLOURS.map((o) =>
      `<option value="${escapeHtml(o.value)}"${o.value === (c?.colour_key || colourKey(c?.name) || 'rtd') ? ' selected' : ''}>${escapeHtml(o.label)}</option>`).join('');
