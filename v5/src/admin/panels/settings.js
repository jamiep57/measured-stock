...
export async function mountSettingsPanel(section = 'users') {
  ...
  function paintWarehouses() {
    ...
    whWrap.querySelectorAll('[data-wh-id]').forEach((btn) => {
      btn.onclick = async () => openWarehouseForm(btn.dataset.whId);
    });
  }
  ...
  function paintCaseSizes() {
    ...
    csWrap.querySelectorAll('[data-cs-id]').forEach((btn) => {
      btn.onclick = async () => openCaseSizeForm(btn.dataset.csId);
    });
  }
  ...
  $('settingsAddWh')?.addEventListener('click', async () => openWarehouseForm(null));
  $('settingsAddCat')?.addEventListener('click', async () => openCategoryForm(null));
  $('settingsAddCs')?.addEventListener('click', async () => openCaseSizeForm(null));
  $('settingsCatMergeToggle')?.addEventListener('click', async () => setMergeMode(true));
  $('settingsCatMergeCancel')?.addEventListener('click', async () => setMergeMode(false));
  $('settingsCatMergeBtn')?.addEventListener('click', async () => openMergeDialog());
  ...
  async function openCaseSizeForm(editId) {
    const cs = editId ? caseSizes.find((x) => x.id === editId) : null;
    const linked = cs ? linkedProducts(cs.id, products).length : 0;
    const suOpts = STOCK_UNIT_OPTS.map((o) =>
      `<option value="${escapeHtml(o.value)}"${o.value === (cs?.stock_unit || 'case') ? ' selected' : ''}>${escapeHtml(o.label)}</option>`).join('');
